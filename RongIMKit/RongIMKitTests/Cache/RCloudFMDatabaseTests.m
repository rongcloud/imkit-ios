...
@interface RCloudFMDatabaseTests : RCFMDBTestBase

@end

@implementation RCloudFMDatabaseTests
...
- (void)testOpenWithVFS {
    ...
    RCloudFMDatabase *db = [[RCloudFMDatabase alloc] initWithPath:@":memory:"];
...
- (void)testURLOpen {
    ...
    RCloudFMDatabase *db = [RCloudFMDatabase databaseWithPath:[fileURL absoluteString]];
...
{
    RCloudFMResultSet *ps = [self.db executeQuery:@"pragma journal_mode=delete"];
...
- (void)testSelectULL
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select a from ull"];
    while ([rs next]) {
...
{
    RCloudFMResultSet *rs = [self.db executeQuery:@"select rowid,* from test where a = ?", @"hi"];
...
{
    RCloudFMResultSet *rs = [self.db executeQuery:@"select rowid, a, b, c from test"];
    ...
    while ([rs next]) {
        ...
        XCTAssertEqual([rs UTF8StringForColumnName:invalidColumnName], (const unsigned char *)0, @"Invalid column name should return nil");
...
- (void)testBusyRetryTimeout
{
    ...
    RCloudFMResultSet *rs = [newDB executeQuery:@"select rowid,* from test where a = ?", @"hi'"];
...
- (void)testCaseSensitiveResultDictionary
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from cs"];
    while ([rs next]) {
...
- (void)testBoolInsert
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from btest"];
    while ([rs next]) {
...
- (void)testNamedParametersCount
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from namedparamcounttest"];
...
- (void)testBlobs
{
    ...
    if (safariCompass) {
        ...
        RCloudFMResultSet *rs = [self.db executeQuery:@"select b from blobTable where a = ?", @"safari's compass"];
        XCTAssertTrue([rs next]);
...
- (void)testNullValues
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from t2"];
    while ([rs next]) {
...
{
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from t3"];
    while ([rs next]) {
        ...
        RCloudFMResultSet *rs2 = [self.db executeQuery:@"select a from t3 where a = ?", [NSNumber numberWithInt:newVal]];
        [rs2 next];
...
- (void)testNSNullInsertion
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from nulltest"];
...
- (void)testNullDates
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from datetest"];
...
- (void)testLotsOfNULLs
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from nulltest2"];
...
- (void)testUTF8Strings
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from utest where a = ?", @"/übertest"];
    XCTAssertTrue([rs next]);
    [rs close];
    XCTAssertFalse([self.db hasOpenResultSets], @"Shouldn't have any open result sets");
    XCTAssertFalse([self.db hadError], @"Shouldn't have any errors");
...
- (void)testArgumentsInArray
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from testOneHundredTwelvePointTwo where b > ?" withArgumentsInArray:[NSArray arrayWithObject:[NSNumber numberWithInteger:1]]];
...
- (void)testColumnNamesContainingPeriods
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select t4.a as 't4.a', t4.b from t4;"];
...
- (void)testFormatStringParsing
{
    ...
    RCloudFMResultSet *rs = [self.db executeQueryWithFormat:@"select * from t5 where a = %s and a = %@ and b = %d", "text", @"text", 42];
    XCTAssertNotNil(rs);
...
- (void)testFormatStringParsingWithSizePrefixes
{
    ...
    RCloudFMResultSet *rs = [self.db executeQueryWithFormat:@"select * from t55 where a = %s order by 2", "a"];
    XCTAssertNotNil(rs);
...
- (void)testFormatStringParsingWithNilValue
{
    ...
    RCloudFMResultSet *rs = [self.db executeQueryWithFormat:@"select * from tatwhat"];
    XCTAssertNotNil(rs);
    XCTAssertTrue([rs next]);
    XCTAssertTrue([rs columnIndexIsNull:0]);
...
{
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from test where a=?" withArgumentsInArray:@[]];
    XCTAssertNil(rs);
}
...
- (void)testDatabaseAttach
{
    ...
    RCloudFMDatabase *dbB = [RCloudFMDatabase databaseWithPath:@"/tmp/attachme.db"];
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from attack.attached"];
    XCTAssertNotNil(rs);
    XCTAssertTrue([rs next]);
    [rs close];
}
...
- (void)testNamedParameters
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from namedparamtest"];
...
{
    RCloudFMResultSet *rs = [self.db executeQuery:@"pragma database_list"];
    int counter = 0;
    while ([rs next]) {
        counter++;
...
- (void)testStatementCachingWorks
{
    ...
    for (int i = 1; i <= 2; i++ ) {
        ...
        RCloudFMResultSet* rs1 = [self.db executeQuery: @"SELECT rowid, * FROM testStatementCaching WHERE value = ?", @1]; // results in 2 rows...
        XCTAssertNotNil(rs1);
        ...
        RCloudFMResultSet* rs2 = [self.db executeQuery:@"SELECT rowid, * FROM testStatementCaching WHERE value = ?", @2]; // results in 1 row
        XCTAssertNotNil(rs2);
...
- (void)testDateFormat
{
    void (^testOneDateFormat)(RCloudFMDatabase *, NSDate *) = ^( RCloudFMDatabase *db, NSDate *testDate ){
        ...
        RCloudFMResultSet *rs = [db executeQuery:@"SELECT test FROM test_format"];
        XCTAssertNotNil(rs);
        XCTAssertTrue([rs next]);
    ...
    NSDateFormatter *fmt = [RCloudFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
- (void)testColumnNameMap
{
    ...
    RCloudFMResultSet *ars = [self.db executeQuery:@"select * from colNameTest"];
    XCTAssertNotNil(ars);
...
- (void)testCharAndBoolTypes
{
    ...
    RCloudFMResultSet *rs = [self.db executeQuery:@"select * from charBoolTest"];
    XCTAssertNotNil(rs);
...
{
    NSString *version = [RCloudFMDatabase sqliteLibVersion];
...
{
    BOOL isThreadSafe = [RCloudFMDatabase isSQLiteThreadSafe];
    XCTAssert(isThreadSafe, @"not threadsafe");
}

- (void)testOpenNilPath
{
    RCloudFMDatabase *db = [[RCloudFMDatabase alloc] init];
...
{
    RCloudFMDatabase *db = [[RCloudFMDatabase alloc] initWithPath:@""];
    XCTAssert([db open], @"open failed");
    XCTAssert([db executeUpdate:@"create table foo (bar text)"], @"create failed");
    NSString *value = @"baz";
    XCTAssert([db executeUpdate:@"insert into foo (bar) values (?)" withArgumentsInArray:@[value]], @"insert failed");
...
- (void)testInvalid
{
    ...
    RCloudFMDatabase *db = [[RCloudFMDatabase alloc] initWithPath:path];
...
- (void)testCloseOpenResultSets
{
    ...
    NSString *value = @"baz";
    XCTAssert([db executeUpdate:@"insert into foo (bar) values (?)" withArgumentsInArray:@[value]], @"insert failed");
    RCloudFMResultSet *rs = [db executeQuery:@"select bar from foo"];
...
- (void)testOpenFailure {
    ...
    RCloudFMDatabase *db = [RCloudFMDatabase databaseWithPath:[fileURL1 absoluteString]];
    ...
    db = [RCloudFMDatabase databaseWithPath:[fileURL2 absoluteString]];
